   ...
   <div class="modal-content">
      ...
      <div class="modal-header border-0 pb-0">
         <div class="form-header modal-header-title text-start mb-0">
            <h6 class="mb-0" >Update Lead Followup</h6>
         </div>
         <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
            <span class="align-center" aria-hidden="true">&times;</span>
         </button>
      </div>
      <form autocomplete="off" method="POST" action="{{ route('followup.leadupdatestatus', ['id' => $Lead_datas['last_call_followupid']]) }}">
      @csrf
               ...
               <div class="col-lg-3 col-md-12">
                  <div class="form-group">
                     ...
                     <select class="form-control select js-example-basic-single" name="lead_id" id="lead_id" required>
                              <option value="{{ $Lead_datas['id'] }}">{{ $Lead_datas['name'] }} </option>
                      </select>
                  </div>
               </div>
